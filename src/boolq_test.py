...
class TestBoolQDataset(unittest.TestCase):
    ...
    def test_item(self):
        ...
        item = self.boolq_dataset[0]
        self.assertEqual(list(item.keys()), ["input_ids", "attention_mask", "labels"])
        self.assertEqual(len(item['input_ids']), self.max_seq_len)
        self.assertIsInstance(item['input_ids'].dtype, type(torch.long))
        self.assertEqual(len(item['attention_mask']), self.max_seq_len)
        self.assertIsInstance(item['attention_mask'].dtype, type(torch.long))
